...
async function Updatedata() {
  ...
  jp_s_records.innerHTML = `<b title="${data.JP_scraping_counter.nowtracking}">${data.JP_scraping_counter.count}</b>(${
    data.JP_scraping_counter.done
  }%) <i style="color:${data.JP_scraping_counter.html.status == 200 ? 'green' : 'red'};">[${data.JP_scraping_counter.html.status}:${
    data.JP_scraping_counter.html.text
  }]</i>`;
  dhl_s_records.innerHTML = `<b title="${data.DHL_scraping_counter.nowtracking}">${data.DHL_scraping_counter.count}</b>(${
    data.DHL_scraping_counter.done
  }%) <i style="color:${data.DHL_scraping_counter.html.status == 200 ? 'green' : 'red'};">[${data.DHL_scraping_counter.html.status}:${
    data.DHL_scraping_counter.html.text
  }]</i>`;
  usps_a_records.innerHTML = `<b title="${data.USPS_API_counter.nowtracking}">${data.USPS_API_counter.count}</b>(${
    data.USPS_API_counter.done
